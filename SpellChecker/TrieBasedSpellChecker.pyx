...
cdef class TrieBasedSpellChecker(NGramSpellChecker):
    ...
    cpdef list candidateList(self, Word word, Sentence sentence):
        ...
        """
        cdef list results
        cdef float penalty_limit
        cdef TrieCandidate candidate, new_candidate
        cdef str candidate_name
        cdef int item_index
        candidates = deque()
        results = []
    ...
    cpdef list generateTrieCandidates(self, TrieCandidate candidate):
        ...
        cdef str current_name, ch2, letters, deasciified_word, replaced, added, added_last, deleted, swapped
        cdef int current_index, j
        cdef float current_penalty
