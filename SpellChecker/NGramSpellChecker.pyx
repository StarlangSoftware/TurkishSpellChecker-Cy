...
cdef class NGramSpellChecker(SimpleSpellChecker):
    ...
    cpdef Word checkAnalysisAndSetRootForWordAtIndex(self,
        ...
        """
        cdef str word_name, upper_case_word_name
        cdef FsmParseList fsm_parses, upper_case_fsm_parses
    ...
    cpdef Word checkAnalysisAndSetRoot(self, str word):
        ...
        """
        cdef FsmParseList fsm_parses_of_word, fsm_parses_of_capitalized_word
    ...
    cpdef Sentence spellCheck(self, Sentence sentence):
        ...
        while i < sentence.wordCount():
            ...
            if self.parameter.isSuffixCheck():
                if self.forcedDeDaSplitCheck(word, result) or self.forcedSuffixSplitCheck(word, result) \
                        or self.forcedQuestionSuffixSplitCheck(word, result):
                    previous_root = self.checkAnalysisAndSetRootForWordAtIndex(result, result.wordCount() - 1)
                    root = next_root
                    next_root = self.checkAnalysisAndSetRootForWordAtIndex(sentence, i + 2)
                    i = i + 1
