...
cdef class ContextBasedSpellChecker(NGramSpellChecker):
    ...
    cpdef list candidateList(self, Word word, Sentence sentence):
        ...
        """
        cdef list words, valid_candidates
        cdef set candidates
        cdef Word w
        cdef FsmParseList parses
        cdef str root, s
        ...
        for w in sentence.getWords():
            if w != word:
                words.append(w)
        for w in words:
            parses = self.fsm.morphologicalAnalysis(Word.toCapital(w.getName()))
            if parses.size() > 0:
    ...
    cpdef int damerauLevenshteinDistance(self, str first, str second):
        ...
        """
        cdef int first_length, second_length, first_index, second_index, cost
